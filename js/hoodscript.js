...
var myCities = [  //NAME AND BOUNDS OF CITIES
    {name: "UK", bnds:[[38.0508,-84.5413],[38.0088,-84.4617]]}
...
var hoodClickGetter = function(ll){
  getJSON.abort();
  getJSON = $.ajax(
    {url:myPath+"/sql?q=SELECT cartodb_id FROM "+tblName+" WHERE ST_Intersects( the_geom, ST_SetSRID((ST_POINT("+ll.lng+", "+ll.lat+")) , 4326)) AND flag = false GROUP BY cartodb_id ORDER BY cartodb_id ASC",
    crossDomain:true,
      dataType:"jsonp",
